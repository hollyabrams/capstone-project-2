      ...
      if (token) {
        try {
          ...
          if (decoded && decoded.username) {
            ...
            ModeApi.token = token;
            let currentUser = await ModeApi.getCurrentUser(username);
...
async function signup(signupData) {
  try {
    let token = await ModeApi.signup(signupData);
    setToken(token);
    ModeApi.token = token;
    let decoded = decodeToken(token);
  ...
  async function login(loginData) {
    try {
      let token = await ModeApi.login(loginData);
      setToken(token);
  ...
  async function deleteUser() {
    try {
      await ModeApi.deleteProfile(currentUser.username);
      logout();
      return { success: true };
    } catch (errors) {
