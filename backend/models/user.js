...
static async addTransaction(username, transactionData) {
  ...
  const result = await db.query(
    `INSERT INTO transactions (user_id, product_id, quantity, total_price)
     VALUES ($1, $2, $3, $4)
     RETURNING transaction_id, user_id, product_id, quantity, total_price, created_at`,
  ...
  /** Given a username, returns transactions associated with a user.
   *
   * Returns { user_id, product_id, quantity, total_price, date }
   *
   **/
